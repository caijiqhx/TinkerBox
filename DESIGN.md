...
- **数据全内置、彻底离线**：`holidays.json`（节假日 / 调休）+ 1900-2100 农历表 + 1900-2100 二十四节气表。
...
- `month` action 返回整月网格（周一开头；每格带 状态[节假日/调休/周末/工作日] / 节日名 /
  农历（短形式 + 完整形式）/ 农历节日 / 节气 / 是否今天）和月份概览（本月节假日、调休天数与跨度）；
  **无数据的年份退化为纯公历**，不报错
- **格子里"当天是什么日子"的展示规则**：有农历节日 / 节气时，用它**顶替**农历日（完整农历在悬停提示里）；
  法定假期名只在**正日子**当天进格子（`fest_day`，如中秋只在中秋节那天，其余天进悬停提示）——
