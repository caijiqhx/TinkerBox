...
- `month` action 返回整月网格（周一开头；每格带 状态[节假日/调休/周末/工作日] / 节日名 /
  ...
  外加**干支纪年**（`ganzhi`，如「丙午马年」）；`year` action 同样带 `ganzhi` 供年度视图用。
  干支是**纯计算**（序号 = (农历年 - 4) % 60，公元 4 年为甲子），不需要数据表；
  口径按**农历年**（正月初一换年）—— 公历月与农历年天然错位，所以用「区间」而不是单点求值：
  跨农历年的月份给两段（「乙巳蛇年 → 丙午马年」）。农历表范围外（1900-01-31 之前、2100 之后）返回空串，
  前端不显示。**无数据的年份退化为纯公历**，不报错
