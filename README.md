...
- **清单**：点「新建清单」**不弹输入框**，直接建出「新清单 1」并**立刻进入改名状态**（想改就改，按 Esc 就用默认名）；之后切到该清单，**点标题也能改**。默认清单「任务」不可改名。自定义清单**可以直接拖动调整顺序**（拖到目标的上半/下半决定插在它前面还是后面），鼠标移上去还会出现 **↑ 置顶** 和 **✕ 删除**；默认清单固定第一，既不参与排序、也没有置顶按钮，已经在最前面的清单同样不显示置顶按钮
