...
- **二十四节气 + 农历传统节日**：跟在格子的农历后面显示（如「初三 · 立秋」「廿九 · 除夕」），比农历更实一点的颜色。**节气表 1900-2100**（与 solarlunar 逐条核对过）；**农历节日是纯算出来的**（元宵 / 龙抬头 / 七夕 / 中元 / 重阳 / 腊八 / 小年 / 除夕），不依赖任何外部数据，内网也全都有。同一天既是节日又是节气时，格子显示节日、另一个进悬停提示；与法定节日名重复的不再重复（清明那天只写「清明节」）
