...
**自动备份**：每次保存前会把上一版轮转成 `todo.json.bak.1 … bak.5`（1 最新、编号越大越旧，超出的自动丢弃，所以份数有界、不需要手工清理）。它防的是"整个文件被写坏 / 被意外覆盖"这类事故；**它防不住"删错了一条任务"** —— 那个要靠回收站（尚未实现）。份数在 `data/config.json` 里用 `backup_keep` 调整，设 `0` 关闭。
