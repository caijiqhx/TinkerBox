...
- 每月网格：周一开头，7 列
- 四种状态着色：**节假日**（红，底纹 + 节日名）、**调休补班**（橙，带「班」徽标）、**周末**（灰）、**工作日**（默认）
...
- 今天：品牌色描边
- **与待办联动**：格子上显示当天到期的待办数（未完成用实心点、已完成用空心点，悬停看明细），**点击有任务的那天**跳到待办的「某天待办」（跨清单，按未完成 / 已完成分组）；没有待办的日子格子不可点（避免误点进空页），改用格子右下角的 **「＋」（鼠标移到格子上才出现）给这天新建任务**，点了一路跳到那天并把光标放进输入框
